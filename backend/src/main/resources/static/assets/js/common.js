      ...
      ${
        user
          ? `<div class="nav-links">
              ...
              <a href="profile.html" class="${active === "profile" ? "active" : ""}" style="position:relative;" id="navBellLink">Profile <span class="nav-bell" id="navBell" style="display:none;"></span></a>
...
function mountNav(active) {
  ...
  if (slot) {
    slot.innerHTML = renderNav(active);
...
function refreshNotificationBadge() {
  ...
  if (!bell || !store.getToken()) return;
  API.notifications()
    .then((n) => {
      bell.textContent = n.count > 9 ? "9+" : String(n.count);
      bell.style.display = n.count > 0 ? "block" : "none";
    })
    .catch(() => {});
}
